...
------------------------------------------------------------------------
Copyright 2015 - 2016 Adrian Gordon
    ...
    private static $slug = 'gravity-forms-wysiwyg-ckeditor';
		...
		// register actions
            if ( self::is_gravityforms_installed() ) {
				...
				add_filter( 'gfpdf_field_class', array( &$this, 'decode_wysiwgy_gravitypdf_4_0' ), 10, 3 );
				...
				if ( $ckeditor_settings['enable_upload_image'] ) {
		...
		public function ckeditor_script_js() {
			$ckeditor_settings = self::get_options();
					...
					$(function(){
						$('.gform_wrapper .gform_wysiwyg_ckeditor:not(.wysiwyg_exclude) textarea:not([disabled="disabled"],.wysiwyg_exclude), .gform_wrapper .gform_page:not([style="display:none;"],.wysiwyg_exclude) .gform_wysiwyg_ckeditor:not(.wysiwyg_exclude) textarea:not([disabled=disabled],.wysiwyg_exclude), #field_settings textarea:not([disabled=disabled],#gfield_bulk_add_input,#field_calculation_formula,.wysiwyg_exclude,#field_placeholder_textarea), .gf_entry_wrap .postbox .gform_wysiwyg_ckeditor:not(.wysiwyg_exclude) textarea:not([disabled=disabled],.wysiwyg_exclude)').each(function() {
							...
							toolbar: [
								...
								if ( rgar( $ckeditor_settings, 'enable_image' ) || rgar( $ckeditor_settings, 'enable_upload_image' ) ) { echo "'Image',";}
							...
							allowedContent: true
							 <?php if ( rgar( $ckeditor_settings, 'enable_upload_image') ) {?>, filebrowserImageUploadUrl  : '<?php echo admin_url('admin-ajax.php'); ?>?action=itsg_gf_wysiwyg_ckeditor_upload&form_id=<?php 
							...
							CKEDITOR.on( 'dialogDefinition', function( event ) {
								...
								if ( dialogName == 'image' ) {
									...
									<?php if ( rgar( $ckeditor_settings, 'enable_upload_image')) {?>
									// handle default tab
				...
				// destroy all existing CKEditor instances when field is delete in form editor - hooks into existing StartDeleteField function.
				...
				// destroy all existing CKEditor instances when fileupload field is switched between single and multi upload in form editor - hooks into existing ToggleMultiFile function.
				// backup original StartDeleteField
		...
		public static function admin_warnings() {
			if ( !self::is_gravityforms_installed() ) {
				$gravityforms_url = '<a href="https://www.e-junkie.com/ecom/gb.php?cl=54585&c=ib&aff=299380" target="_blank" >' . __( 'download the latest version of Gravity Forms', 'gravity-forms-wysiwyg-ckeditor' ) . '</a>';
				...
				printf(
					'<div class="error"><h3>%s</h3><p>%s</p><p>%s</p></div>',
						...
						sprintf ( __( 'Please %s and try again.', 'gravity-forms-wysiwyg-ckeditor' ), $gravityforms_url )
				);
			}
		} // END admin_warnings
		...
		public static function admin_warnings_noconflict() {
			$ckeditor_settings_url = '<a href="' . admin_url( 'admin.php?page=gf_settings&subview=WYSIWYG+ckeditor' ) . '" >' . __( 'CKEditor Settings', 'gravity-forms-wysiwyg-ckeditor' ) . '</a>';
			
			$gf_settings_url = '<a href="' . admin_url( 'admin.php?page=gf_settings' ) . '" >' . __( 'Gravity Forms Settings', 'gravity-forms-wysiwyg-ckeditor' ) . '</a>';
			...
			printf(
				'<div class="error"><h3>%s</h3><p>%s</p><p>%s</p><p>%s</p>%s</div>',
					...
					sprintf ( __( "No-Conflict Mode is currently 'On' in the %s page.", 'gravity-forms-wysiwyg-ckeditor' ), $gf_settings_url ),
					...
					sprintf ( __( "Set the 'No-Conflict Mode' option to 'Off' in the %s page.", 'gravity-forms-wysiwyg-ckeditor' ), $gf_settings_url ),
					sprintf ( __( "Disable the 'Enable in form editor' option in the %s page.", 'gravity-forms-wysiwyg-ckeditor' ), $ckeditor_settings_url ),
					sprintf ( __( "Enable the 'Override Gravity Forms No-Conflict Mode' option in the %s page.", 'gravity-forms-wysiwyg-ckeditor' ), $ckeditor_settings_url )
					)
			);
