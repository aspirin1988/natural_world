	 ...
	 public static function settings_page(){
		   ...
		   $ckeditor_settings = ITSG_GF_WYSIWYG_CKEditor::get_options();
			...
			/* ENABLE IN FORM EDITOR */
				...
				$ckeditor_settings['enable_override_gfnoconflict'] = rgpost( 'override_gfnoconflict' );
			/* SOURCE */
			...
			<form method="post">
				...
				<?php if ( get_option( 'gform_enable_noconflict' ) ) { 
					...
					?>
					<p>
						<?php printf (__( "<strong>IMPORTANT</strong>: No-Conflict Mode is currently 'On' in the %s page.", 'gravity-forms-wysiwyg-ckeditor' ), $gf_settings_url) ?>
					</p>
					<p>
						<?php _e( 'No-Conflict Mode allows Gravity Form users to disable all third party scripts from running in Gravity Form admin pages - <strong>this will stop CKEditor WYSIWYG for Gravity Forms from working in these pages</strong>.', 'gravity-forms-wysiwyg-ckeditor' ) ?>
					</p>
					<?php printf( "%s:<ol><li style='list-style-type: decimal'>%s</li><li style='list-style-type: decimal'>%s</li><li style='list-style-type: decimal'>%s</li></ol>", 
					...
					<input type="checkbox" id="override_gfnoconflict" name="override_gfnoconflict" <?php echo rgar( $ckeditor_settings, 'enable_override_gfnoconflict' ) ? "checked='checked'" : "" ?>  >
					<label for="override_gfnoconflict"><?php _e( 'Override Gravity Forms No-Conflict Mode', 'gravity-forms-wysiwyg-ckeditor' ) ?></label>
				<?php } ?>
