<?php
/**
 * natural functions and definitions.
 *
...
function natural_setup() {
	 ...
	 * If you're building a theme based on natural, use a find and replace
	 * to change 'natural' to the name of your theme in all the template files.
	 */
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', 'natural' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'natural_custom_background_args', array(
...
function natural_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'natural_content_width', 640 );
}
...
function natural_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'natural' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'natural' ),
 ...
 */
function natural_scripts() {
	wp_enqueue_style( 'natural-style', get_stylesheet_uri() );

	wp_enqueue_script( 'natural-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'natural-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'natural_scripts' );
